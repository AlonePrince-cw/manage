  ...
  <el-dialog :visible.sync="visible" :title="title" width="800px" @open="onOpen" class="myDialog">
    ...
    >
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="成本金额：" prop="occurAmount">
            <el-input v-model.number="searchParams.occurAmount" placeholder="请输入金额"></el-input>
          </el-form-item>
        </el-col>
        <!-- <el-col :span="12">
...
import { costIncrease, costModification } from "@/api/costRecord";
  ...
  data() {
    return {
      ...
      { required: true, message: "请选择类型", trigger: "blur" }
        ...
        occurAmount: [
          { required: true, message: "请填写成本金额", trigger: "blur" }
        ]
      },
      attachmentFiles: [],
  ...
  methods: {
    ...
    handleAdd() {
      ...
      vm.$refs.searchParams.validate().then(async () => {
        ...
        await costIncrease(vm.searchParams);
        vm.loading = false;
    ...
    handleEdit() {
      ...
      vm.$refs.searchParams.validate().then(async () => {
        ...
        await costModification(vm.searchParams);
        vm.loading = false;
