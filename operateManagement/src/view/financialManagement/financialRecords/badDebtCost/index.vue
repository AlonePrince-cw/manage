<template>
  <div>
    <el-tabs type="border-card" v-model="targerName">
      <el-tab-pane label="坏账成本" name="first">
        <el-form
          class="dz-toolbar"
          ref="searchForm"
          :inline="true"
        ...
        >
          ...
          <el-form-item label="金额范围:" class="meneyArea">
            <el-input v-model="searchParams.badDebtStartAmount" placeholder="请输入金额"></el-input>
            <span> - </span>
            <el-input v-model="searchParams.badDebtEndAmount" placeholder="请输入金额"></el-input>
          </el-form-item>
...
<!--           </el-form-item>-->
        ...
        <el-col :span="24">
          <div class="fl">
            <el-button type="primary" @click="addPlan" class="reateBtn" v-if="permButton('71010801')">创建坏账成本</el-button>
            <el-button type="primary" @click="importExcel" class="reateBtn" v-if="permButton('71010808')">导入</el-button>
          </div>
        </el-col>
        <el-table border :data="expenseRecordData">
          ...
          <el-table-column
            ...
            align="center"
          ></el-table-column>
          <el-table-column label="关联项目" :show-overflow-tooltip="true" align="center" width="180px">
            <template slot-scope="scope">
...
<!--          >-->
<!--         <template slot-scope="scope">-->
<!--           {{scope.row.costType | dict("costType")}}-->
<!--         </template>-->
<!--          </el-table-column>-->
          <el-table-column label="操作" width="250px" fixed="right" align="center">
            <template slot-scope="scope">
              <el-button size="small" type="text" @click="check(scope.row)" v-if="permButton('71010802')">查看</el-button>
              <el-button size="small" type="text" @click="update(scope.row)" v-if="permButton('71010803')">编辑</el-button>
              <el-button
              ...
              ><span v-if="permButton('71010807')">设置失效</span></el-button>
              <el-button @click="validChange(scope.row)" v-else size="small" type="text" ><span v-if="permButton('71010807')">设置有效</span></el-button>
            </template>
          </el-table-column>
        </el-table>
        <Pager
...
import { badDebtCostPage, badDebtCostEffectiveness } from "@/api/badDebtCost";
...
export default {
  ...
  data() {
    return {
      ...
      upLoad:[],
      uploadPath: '/api/dispatch/pc/71010808',
      importTitle: '批量导入坏账成本',
      // templateId: '8ff577d3b5834aa3aff47527fd9d3ce8',//正式140环境
      templateId: 'f4e1648cc3434e4586c085aba9e684ce',//测试环境 3900a5e54e32444d9238e8dbf72ce6ba
      expenseRecordData: [],
      searchParams: {
        page: 1,
        size: 10,
        ...
        occurEndTime:"",//发生结束时间
        badDebtStartAmount:"",//坏账成本开始金额
        badDebtEndAmount:"",//坏账成本结束金额
        // costType:"",//成本类型
  ...
  methods: {
    ...
    validChange(v) {
      ...
      vm.$confirm(
        ...
        try {
          await badDebtCostEffectiveness(vm.isValidParams);
    ...
    async fetchTableData() {
      ...
      try {
        ...
        let data = await badDebtCostPage(vm.searchParams);
        console.log(data);
    ...
    Reset() {
      this.searchParams = {
        ...
        occurStartTime:'',	//发生开始时间
        occurEndTime:"",//发生结束时间
        badDebtStartAmount:"",//成本开始金额
        badDebtEndAmount:"",//成本结束金额
        // costType:"",//成本类型
        projectId: "", //	关联项目id
        relatedWork: "", //	关联工作
        employeeId: "", //	负责人id
