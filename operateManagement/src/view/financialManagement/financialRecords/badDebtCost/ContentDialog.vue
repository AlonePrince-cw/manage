  ...
  <el-dialog :visible.sync="visible" :title="title" width="800px" @open="onOpen" class="myDialog">
    ...
    >
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="坏帐金额：" prop="badDebtAmount">
            <el-input v-model.number="searchParams.badDebtAmount" placeholder="请输入金额"></el-input>
          </el-form-item>
        </el-col>
<!--        <el-col :span="12">-->
...
<!--        </el-col>-->
    ...
    <el-row slot="footer" class="dz-dialog-footer">
      <el-button
        ...
        type="primary"
        @click="handleAdd()"
        v-if="dialogHandleType === 'add' && permButton('71010801')"
      >确认创建</el-button>
      <el-button
        class="button"
        type="primary"
        @click="handleEdit()"
        v-if="dialogHandleType === 'edit' && permButton('71010803')"
      >保存修改</el-button>
    </el-row>
  </el-dialog>
</template>
...
import { badDebtCost, badDebtCostEdit } from "@/api/badDebtCost";
  ...
  methods: {
    ...
    handleAdd() {
      ...
      vm.$refs.searchParams.validate().then(async () => {
        ...
        await badDebtCost(vm.searchParams);
        vm.loading = false;
    ...
    handleEdit() {
      ...
      vm.$refs.searchParams.validate().then(async () => {
        ...
        await badDebtCostEdit(vm.searchParams);
        vm.loading = false;
