...
<!--        </el-col>-->
    ...
    <el-row slot="footer" class="dz-dialog-footer">
      <el-button
        ...
        type="primary"
        @click="handleAdd()"
        v-if="dialogHandleType === 'add' && permButton('71010601')"
      >确认创建</el-button>
      <el-button
        class="button"
        type="primary"
        @click="handleEdit()"
        v-if="dialogHandleType === 'edit' && permButton('71010603')"
      >保存修改</el-button>
    </el-row>
  </el-dialog>
</template>
...
import { taxRevenue, taxRevenueEdit } from "@/api/taxRevenue";
  ...
  methods: {
    ...
    handleAdd() {
      ...
      vm.$refs.searchParams.validate().then(async () => {
        ...
        await taxRevenue(vm.searchParams);
        vm.loading = false;
    ...
    handleEdit() {
      ...
      vm.$refs.searchParams.validate().then(async () => {
        ...
        await taxRevenueEdit(vm.searchParams);
        vm.loading = false;
