<template>
  <el-dialog :visible.sync="visible" :title="title" width="800px" @open="onOpen" class="myDialog">
    ...
    >
      <el-row>
        <el-col :span="12">
          <el-form-item label="收款时间：" prop="gatheringTime">
            <el-date-picker
              v-model="searchParams.gatheringTime"
              type="date"
              placeholder="选择日期"
        ...
        <el-col :span="12">
          <el-form-item label="收款金额：" prop="gatheringAmount">
            <el-input v-model.number="searchParams.gatheringAmount" placeholder="请输入金额"></el-input>
          </el-form-item>
        </el-col>
        <!-- <el-col :span="12">
          <el-form-item label="费用类型：">
            <GetPlatformDict
              :dictType="'expenseType'"
              :disabled="dialogHandleType === 'check'"
              :value="searchParams.expenseType"
              :snycModel.sync="searchParams.expenseType"
            ></GetPlatformDict>
          </el-form-item>
        </el-col>
        <el-col :span="12">
    ...
    <el-row slot="footer" class="dz-dialog-footer">
      <el-button
        ...
        type="primary"
        @click="handleAdd()"
        v-if="dialogHandleType === 'add' && permButton('71010301')"
      >确认创建</el-button>
      <el-button
        class="button"
        type="primary"
        @click="handleEdit()"
        v-if="dialogHandleType === 'edit' && permButton('71010303')"
      >保存修改</el-button>
    </el-row>
  </el-dialog>
</template>
...
import { confirmCreation, modifyExpenseRecord } from "@/api/collectionRecords";
