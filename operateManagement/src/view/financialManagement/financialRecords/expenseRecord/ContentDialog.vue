<template>
  <el-dialog
      ...
      <el-row>              
        ...
        <el-col :span="12">
            <el-form-item label="费用金额：" prop="expenseAmount">
              <el-input v-model.number="searchParams.expenseAmount" placeholder="请输入金额"></el-input>
          </el-form-item>
        </el-col>
           <el-col :span="12">
        <el-form-item label="费用类型：" prop="expenseType">
            <GetPlatformDict  :dictType = "'expenseType'" :disabled="dialogHandleType === 'check'" :value="searchParams.expenseType" :snycModel.sync="searchParams.expenseType"></GetPlatformDict>
        </el-form-item>
           </el-col>
        <el-col :span="12">
    ...
    <el-row slot="footer" class="dz-dialog-footer">
      <el-button class="button" type="primary" @click="handleAdd()" v-if="dialogHandleType === 'add' && permButton('71010201')">确认创建</el-button>
      <el-button class="button" type="primary" @click="handleEdit()" v-if="dialogHandleType === 'edit' && permButton('71010203')">保存修改</el-button>
    </el-row>
  </el-dialog>
</template>
...
export default {
  ...
  computed: {
    title () {
      return this.dialogHandleType === 'add' ? '创建费用记录' : this.dialogHandleType === 'edit' ? '修改费用记录' : '创建费用记录'
    },
  ...
  methods: {
    ...
    handleAdd () {
      ...
      vm.$refs.searchParams.validate().then( async() => {
        ...
        await confirmCreation(vm.searchParams)
        vm.loading = false
    ...
    handleEdit () {
      ...
      vm.$refs.searchParams.validate().then( async() => {
        ...
        await modifyExpenseRecord(vm.searchParams)
        vm.loading = false
