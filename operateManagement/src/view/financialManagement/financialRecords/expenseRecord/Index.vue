<template>
  <div>
    <el-tabs type="border-card" v-model="targerName">
      <el-tab-pane label="费用记录" name="first">
        <el-form
          class="dz-toolbar"
          ref="searchForm"
          :inline="true"
        ...
        >
          ...
          <el-form-item label="金额范围:" class="meneyArea">
            <el-input v-model="searchParams.expenseStartAmount" placeholder="请输入金额"></el-input>
            <span>-</span>
            <el-input v-model="searchParams.expenseEndAmount" placeholder="请输入金额"></el-input>
          </el-form-item>
...
<!--          </el-form-item>-->
        ...
        <el-col :span="24">
          <div class="fl">
            <el-button type="primary" @click="addPlan" class="reateBtn" v-if="permButton('71010201')">创建费用记录</el-button>
            <el-button type="primary" @click="importExcel" class="reateBtn" v-if="permButton('71010208')">导入</el-button>
          </div>
        </el-col>
        <el-table border :data="expenseRecordData">
          ...
          <el-table-column label="关联项目" :show-overflow-tooltip="true" align="center" width="180px">
            <template slot-scope="scope">
              <span v-if="scope.row.associationType === 0">{{scope.row.project.projectName}}</span>
              <span v-else>--</span>
            </template>
          </el-table-column>
         ...
         <template slot-scope="scope">
          ...
          <el-table-column label="操作" width="250px" fixed="right" align="center">
            <template slot-scope="scope">
              <el-button size="small" type="text" @click="check(scope.row)" v-if="permButton('71010202')">查看</el-button>
              <el-button size="small" type="text" @click="update(scope.row)" v-if="permButton('71010203')">编辑</el-button>
              <el-button
              ...
              ><span v-if="permButton('71010207')">设置失效</span></el-button>
              <el-button @click="validChange(scope.row)" v-else size="small" type="text"><span v-if="permButton('71010207')">设置有效</span></el-button>
            </template>
          </el-table-column>
        </el-table>
        <Pager
...
import { expenseRecord, costEffective } from "@/api/expenseRecord";
...
export default {
  ...
  data() {
    return {
      ...
      uploadPath: '/api/dispatch/pc/71010208',
      importTitle: '批量导入费用记录',
      // templateId: '5ab9833393954d12bc4d19116e173831',//正式环境140
      templateId: '6f63ef08f9eb4030a1a995e15920407e',//
      expenseRecordData: [],
      searchParams: {
        page: 1,
        size: 10,
        ...
        occurEndTime:"",//	发生结束时间
        expenseStartAmount:"",//金额开始时间
        expenseEndAmount:"",//金额结束时间
  ...
  methods: {
    ...
    async fetchTableData() {
      ...
      try {
        ...
        let data = await expenseRecord(vm.searchParams);
        console.log(data);
