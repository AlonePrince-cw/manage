<template>
    <el-dialog
        ...
        <el-row slot="footer" class="dz-dialog-footer">
            <el-button class="button" type="primary" @click="handleAdd()" v-if="dialogHandleType === 'add' && permButton('11030201')">确认创建
            </el-button>
            <el-button class="button" type="primary" @click="handleEdit()" v-if="dialogHandleType === 'edit' && permButton('11030203')">保存修改
            </el-button>
        </el-row>
        <Uploader
            ref="uploaders"
...
<script>
  ...
  import { penaltyAdd, penaltyEdit } from '@/api/rewardManage'
  ...
  export default {
    ...
    methods: {
      ...
      handleAdd () {
        ...
        try {
          vm.$refs.addDialogFrom.validate().then(async () => { // 表单验证
            await penaltyAdd(vm.formParams)
            vm.$emit('success')
      ...
      handleEdit () {
        ...
        try {
          vm.$refs.addDialogFrom.validate().then(async () => { // 表单验证
            await penaltyEdit(vm.formParams)
            vm.$emit('success')
