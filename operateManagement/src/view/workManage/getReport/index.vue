<template>
  <div>
    <el-tabs type="border-card" v-model="targerName">
      <el-tab-pane label="收到的日报" name="first">
        <el-form
          class="dz-toolbar"
          ref="searchForm"
          :inline="true"
          ...
          <el-form-item label="姓名:">
            <el-input v-model="searchParams.employeeName" placeholder="请输入姓名"></el-input>
          </el-form-item>
          ...
          <el-form-item label="关联方式">
            <el-select v-model="searchParams.associationType" @change="associationTypeChange">
        ...
        <el-card
          ...
          <el-col :span="24" class="reportInfo">
            <el-col :span="2" style="width: 50px">
              <img src="../../../assets/avatar.png" @error="errorImg($event,'avatar')"/>
            </el-col>
            <el-col :span="22">
            ...
            <el-col :span="22" style="margin-top: 3px">
              ...
              <span @click="receiveRange(item)" style="color: #409EFF;margin-left: 16px;cursor: pointer" v-if="permButton('510407')">接收范围</span>
            </el-col>
            <el-button type="danger" class="readReport" v-if="item.replyStatus !== 1" size="small" @click="readReport(item.id)"><span v-if="permButton('510403')">未阅读</span></el-button>
            <el-button type="success" class="readReport" v-else size="small"><span v-if="permButton('510403')">已阅</span></el-button>
          </el-col>
          <el-col :span="24">
            <el-col :span="19" class="line2">
              <span v-if="item.associationType === 0">
                <el-tag size="medium" style="padding: 0px 10px">{{item.hstoneProject.projectName}}</el-tag>
                <span type="success" style="margin-left: 16px">关联计划：无</span>
              </span>
              <span v-if="item.associationType === 1">
                <el-tag size="medium" type="success" style="padding: 0px 10px">{{item.relatedWorkDictionary.value}}</el-tag>
          ...
          <el-col :span="24" class="read" v-if="item.readEmployees.length !== 0">
            ...
            <el-avatar class="header-img" :size="40" :src="item.headImg" v-for="(v,n) in item.readEmployees" :key="v.id"></el-avatar>
          </el-col>
          <el-col :span="24">
            <commentReply :mainId="item.id" :comments="replyFn(item.hstoneWorkDayReportComments)" @success="refreshTable"></commentReply>
          </el-col>
        </el-card>
...
import { getReportPage, readReport, reportRange } from "@/api/report";
export default {
  components: {},
  data() {
    return {
  ...
  methods: {
    ...
    readReport (v) {
      ...
      vm.$confirm(`是否标记已阅读？`, '提示', {
          type: 'warning'
      }).then(async () => {
          await readReport(params)
    ...
    associationTypeChange (v){
      if(v === 0){
        this.searchParams.relatedWork = null
      }else {
    ...
    async fetchTableData() {
      ...
      try {
        let data = await getReportPage(vm.searchParams)
