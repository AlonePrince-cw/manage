<template>
  <div>
    <el-tabs type="border-card" v-model="targerName">
      <el-tab-pane label="收到的计划" name="first">
        <el-form
          class="dz-toolbar"
          ref="searchForm"
          :inline="true"
...
<!--          <el-form-item label="计划内容：" prop="info">-->
<!--            <el-input v-model="searchParams.info" placeholder="请输入计划内容"></el-input>-->
<!--          </el-form-item>-->
        ...
        <el-table border :data="planData">
          <el-table-column label="计划时间" :show-overflow-tooltip="true" width="120px" prop="name" align="center">
            <template slot-scope="scope">
              {{scope.row.planTime | dateFormat('yyyy-MM-dd')}}
            </template>
          </el-table-column>
...
<!--          <el-table-column label="计划内容" :show-overflow-tooltip="true" prop="info" align="center"></el-table-column>-->
...
import { planGetPage, planDel } from "@/api/plan";
...
export default {
  ...
  methods: {
    ...
    async fetchTableData() {
      ...
      try {
        let data = await planGetPage(vm.searchParams)
