<template>
  <el-dialog
    ...
    <el-form ref="myForm" :inline="true" :model="menulistParams"  class="demo-form-inline" label-width="100px" style="padding: 0 10px;">
      ...
      <el-form-item label="上级目录：">
        <MenusAllSelect
            ...
            retryText="点击重试?"
            v-model="menulistParams.parentId"
    ...
    <el-row slot="footer" class="dz-dialog-footer">
      ...
      <el-button class="button" type="primary" @click="handleAlter()" v-if="dialogHandleType === 'edit'">保存修改</el-button>
    </el-row>
  </el-dialog>
</template>
...
import { delrolePrem, permDelListPage } from '@/api/role'
...
export default {
  ...
  computed: {
    ...
    title () {
      return this.dialogHandleType === 'add' ? '解除关联权限' : this.dialogHandleType === 'edit' ? '编辑关联权限' : '查看关联权限'
    }
  },
  methods: {
    menuIdChange(v) {
    ...
    async fetchTableData () {
      let vm = this
      vm.loading = true
      vm.menulistParams.roleId = vm.role.role_checked[0].id
      let data = await permDelListPage(vm.menulistParams)
    ...
    async handleAdd () {
      ...
      try {
        vm.role.role_add_params.roleId = vm.role.role_checked[0].id
        let arr = []
        ...
        await delrolePrem(vm.role.role_add_params)
