...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.msg == 'SUCCESS') { // 修改数据返回状态解析
                        _this.imageList.push(json.data[0]); // 修改数据返回状态解析
        ...
        getInsertList: function () {
            var i, data, list = [],
                align = getAlign(),
                prefix = editor.getOpt('imageUrlPrefix'); //自定义图片访问前缀
                serverUrlPrefix = editor.getOpt('serverUrlPrefix'); //自定义文件访问前缀
            for (i = 0; i < this.imageList.length; i++) {
                data = this.imageList[i];
                list.push({
                    src: serverUrlPrefix + prefix + data.uniqueCode, //拼接富文本访问图片路径
                    _src: serverUrlPrefix + prefix + data.uniqueCode,
