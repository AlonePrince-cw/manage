    ...
    UploadFile.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                var $file = $('#' + file.id);
                var prefix = editor.getOpt('imageUrlPrefix'); //自定义图片访问前缀
                var serverUrlPrefix = editor.getOpt('serverUrlPrefix'); //自定义文件访问前缀
                try {
                    var responseText = (ret._raw || ret),
                        json = utils.str2json(responseText);
                    if (json.msg == 'SUCCESS') {
                        uploadVideoList.push({
                            'url': serverUrlPrefix + prefix + json.data[0].uniqueCode +'&url='+ json.data[0].extension,
                            'type': json.data[0].extension,
                            'original':json.data[0].extension,
                        });
